<template>
  <n-space>
    <n-radio-group v-model:value="pickValue">
      <n-radio-button v-for="item in datePickList" :value="item.value" :key="item.value" :label="item.label" />
    </n-radio-group>
    <n-date-picker v-model:value="customRange" type="daterange" :update-value-on-close="true" />
    <n-button type="primary" :loading="loading" @click="handlePickTodayAction">爬取当日异动数据</n-button>
  </n-space>
</template>
